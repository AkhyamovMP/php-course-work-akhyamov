...
function getMessages($user, $channel = '', $tag = '', $topic = '')
{
    $result = '';
    $mainRequest = '
SELECT body, username, private, Messages.owner, C.name AS channel, H.name AS hashtag, title AS topic, T.id AS topic_id, dispatch_time
FROM Messages
...
WHERE private = 0 '; //Пробел в конце очень важен
...
FROM Messages
         ...
         JOIN TopicHashtags TH ON Messages.hashtag = TH.hashtag
         JOIN Topics T ON TH.topic = T.id
WHERE private = 1 AND Messages.owner = ' . getUserByName($user)->fetch_assoc()['id'] . ' ';

    $mainRequestSmall = '
SELECT body, username, private, Messages.owner, C.name AS channel, H.name AS hashtag, dispatch_time
FROM Messages
         JOIN Hashtags H ON Messages.hashtag = H.id
         JOIN Users U ON Messages.owner = U.id
         JOIN Channels C ON Messages.channel = C.id 
...
WHERE private = 1 AND Messages.owner = ' . getUserByName($user)->fetch_assoc()['id'] . ' ';
    ...
    if (!$channel && !$tag && !$topic) { // Пустой запрос - вывод всех сообщений
        $result = ($mainRequestSmall . ' UNION ' . $mainRequestSmall_private);

    } elseif ($channel && !$tag && !$topic) { //Поиск только по каналу
        $result =
            $mainRequestSmall .
            ' AND C.id = ' . getChannelByName($channel)->fetch_assoc()['id'] .
            ' UNION ' .
            $mainRequestSmall_private .
            ' AND C.id = ' . getChannelByName($channel)->fetch_assoc()['id'];
        ...
        $result =
            $mainRequestSmall .
            ' AND Messages.hashtag = ' . getTagByName($tag)->fetch_assoc()['id'] .
            ' UNION ' .
            $mainRequestSmall_private .
            ' AND Messages.hashtag = ' . getTagByName($tag)->fetch_assoc()['id'];
